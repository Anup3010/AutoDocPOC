...
def get_diff_for_commit(repo_path: str = ".", commit_hash: str = "HEAD") -> List[FileChange]:
    ...
    file_statuses = {}
    for line in stdout.splitlines():
        # Strip \r for Windows CRLF line endings
    ...
    changed_files = []
    for filepath, status in file_statuses.items():
        # --text flag forces git to treat file as text not binary
        diff_out, _, _ = run_git(
            ["show", "--text", commit_hash, "--", filepath],
            cwd=repo_path
        )

        # Strip \r from diff output
        diff_out = diff_out.replace('\r\n', '\n').replace('\r', '\n')

        # Count actual additions and deletions from diff lines
        ...
        # Use actual counts — more reliable than --stat on Windows
